...
var TreeRouter = Router.extend({
  ...
  _addRoute: function(routeConfig) {
    ...
    this.routeParams[origRoute] = _.map(routeStr.match(namedParam), function (param) {
  ...
  _processChildren: function (parentValue, children, requestData) {
    ...
    children.forEach(function(childObj, i) {
      ...
      var childData = _.omit(childObj, ['create', 'mount', 'children']);
      childData._index = i;

      var currentValue = create(parentValue, _.extend({}, requestData), childData);
      var isPromise = currentValue instanceof Promise;

      if (isPromise === false) {
        // sync processing
        router._processChildren(currentValue, childObj.children, requestData);
        mount(parentValue, currentValue, _.extend({}, requestData), childData);
        return;
      }
      ...
      currentValue.then(obj => {
        ...
        mount(parentValue, obj, _.extend({}, requestData), childData);
      })
      .catch(err => {
